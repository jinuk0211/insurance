...
import { CODEF_DATASETS, isCodefDatasetKey, normalizeTelecom } from "./codef-dataset-definitions.ts"
import { buildCodefDatasetParams, codefDatasetEndpoint } from "./codef-dataset-params.ts"
...
test("builds period and non-period public institution requests", () => {
  const medicalHistory = buildCodefDatasetParams({
  ...
  assert.equal(medicalHistory.userName, "홍길동")
  assert.equal(medicalHistory.phoneNo, "01012345678")
  assert.equal(medicalHistory.startDate, "20250101")
  assert.equal(medicalHistory.endDate, "20251231")
  assert.equal(medicalHistory.telecom, "2")

  for (const datasetKey of ["nps_expected", "nps_history"]) {
    const params = buildCodefDatasetParams({
      datasetKey,
      userName: "홍길동",
      birthDate: "19900101",
...
test("uses blank optional medical dates and rejects an unknown dataset", () => {
  const params = buildCodefDatasetParams({
    datasetKey: "medical_treatment",
    userName: "홍길동",
    birthDate: "19900101",
  ...
  assert.equal(params.startDate, "")
  assert.equal(params.endDate, "")
  assert.throws(
    () => buildCodefDatasetParams({
      datasetKey: "unknown",
      userName: "홍길동",
      birthDate: "19900101",
      phoneNo: "01012345678",
...
test("validates dataset keys, telecom branches and endpoint lookup", () => {
  assert.equal(isCodefDatasetKey("medical_history"), true)
  assert.equal(isCodefDatasetKey("unknown"), false)
  assert.equal(normalizeTelecom("1"), "1")
  assert.equal(normalizeTelecom("2"), "2")
  assert.equal(normalizeTelecom("unexpected"), "0")
  assert.equal(codefDatasetEndpoint({ datasetKey: "nps_expected" }), CODEF_DATASETS.nps_expected.endpoint)
})
