...
test("attaches the collected source page to a curated rule assessment", () => {
  const result = evaluateCancerScenario(contract(), {
    diagnosisType: "general_cancer",
    diagnosisDate: "2025-02-01",
  })

  assert.match(result.sourceDocument, /신한SOL암보험/)
  assert.equal(result.sourcePage, 2)
})

test("uses an exact catalog document as a provisional rule for a non-curated product", () => {
  const result = evaluateCancerScenario(contract({
    name: "(무)e로운 암보험(해약환급금 미지급형)(2601)",
    company: "DB생명",
    startDate: "2026-01-01",
  }), {
    diagnosisType: "general_cancer",
    diagnosisDate: "2027-02-01",
  })

  assert.equal(result.ruleStatus, "provisional")
  assert.match(result.sourceDocument, /e로운 암보험/)
  assert.equal(result.sourcePage, 7)
  assert.equal(result.candidateAmount, null)
  assert.ok(result.checks.includes("자동 추출 규칙 원문 검토"))
})
