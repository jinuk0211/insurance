...
A separate loan-reference repair raises visible paragraph/category
evidence retention from 80.7\% to 100\%, without establishing relevance.
Reusing identical-pool preparation reduces local first-pass retrieval time
by 54.3\% on 1,508 diagnostic queries without changing returned rows.
These pilot components motivate distinct execution and semantic gates;
full-cohort downstream comparisons and human validation remain incomplete.
\end{abstract}
...
The 12-document original-task pilot contains 96 draft findings: 13 KR,
82 card and one loan. All 96 quotations match their actual source;
two KR drafts are rejected by the original validator, leaving 94 final
report rows. These are traceability checks, not legal accuracy or
first-attempt success. Three loan outputs are empty and remain in the
...
\subsection{Reference-window repair}
The original loan pool tags full paragraphs but stores only their first
1,500 characters. Across all 1,000 loans, only 2,351/2,913 original
paragraph/category pairs retain visible keyword evidence (80.7\%).
Exact keyword-bearing windows restore all 2,913 pairs. A compact window
subset selected on the existing 22 development loans retains these pairs
with 10.7\% fewer indexed characters than full windows
(Appendix~\ref{sec:reference-repair}). It can omit other same-category
clauses; no new pool is admitted to the runtime on lexical gates alone.
These are manually engineered repairs, not an executed Stage~3 outer loop.

Separately, reusing pool tokenization and BM25 construction preserves the
original query expansion, query-dependent IDF and MMR. On 1,508 diagnostic
queries from 883 loans, the original pool's first-pass retrieval time falls
from 35.30 to 16.14 seconds including cold preparation (54.3\%). Across
three pools and three repetitions, all 13,572 paired results match exactly.
These are shared-desktop retrieval times, not downstream accuracy or
end-to-end model latency (Appendix~\ref{sec:reference-repair}).
...
The current execution envelope cannot fund full-cohort inference
(Appendix~\ref{sec:implementation}). We therefore report pilot components
separately from the retained 3,000-source roster. Completion requires
budget reconciliation and a fit evaluator, not a success-only dataset.
...
source-window repair fixes a lexical reference defect; preparation reuse
reduces local retrieval time while preserving identical-pool outputs.
...
The reference-window repair retains category keywords, not every clause
occurrence or exception. Its motivating defect was observed in earlier
all-source diagnostics; reserved inputs are not untouched held-out data.
...
\section{Pilot Source and Reference Audits}
\label{sec:reference-repair}
The latest four completed originals per task yield 96 draft flags and
94 final report rows. Source and extracted-text hashes for all 12 documents,
40 stage-artifact hashes and all 96 literal quotations were rechecked.
The final KR rows link to retained validation records; two rejected drafts
remain in the ledger. Latest completion is not pass@1: an earlier loan
failure is preserved and the current loan outputs follow a recorded rerun.

\begin{table*}[t]
\centering\small\setlength{\tabcolsep}{5pt}
\begin{tabular}{lrrr}
\toprule
Metric & Original & Full windows & Compact cover \\
\midrule
Original paragraph/category pairs & 2,913 & 2,913 & 2,913 \\
...
\bottomrule
\end{tabular}
\caption{Verified lexical reference audit across all 1,000 loans, including
129 original zero-chunk sources and one quality-flagged input. Repeated
windows do not inflate the fixed paragraph/category denominator.
Neither keyword retention nor indexed size is relevance or measured latency.}
\label{tab:reference-repair}
\end{table*}

Table~\ref{tab:reference-repair} preserves original paragraph eligibility
and the first-40-eligible-paragraph cap. Full windows are exact source spans
of at most 1,500 characters with 24-character overlap, retaining only
keyword-bearing windows. Compact cover greedily selects windows covering
all original parent categories; ties favor earlier source positions.
Its complete-retention and visible-tag gates select the smallest development
index before application to the 978 reserved loans. Development indexed
characters are 79,667/93,220/79,856 for original/full/compact: compact is
slightly larger than original there. Over all loans it saves only 0.41\%
against original, versus 10.68\% against full windows, and adds 80 chunks.
All 8,146 candidate chunks were checked against their exact raw source spans.
These measurements justify a literal-evidence repair, not semantic completeness,
new relevance labels, runtime pool adoption or an automatic improvement curve.

\paragraph{Prepared retrieval.}
The original extractor generates at most three taxonomy-triggered queries
per source: 883 loans yield 1,508 queries and 117 zero-query sources remain
explicit. There are 31 development-linked and 1,477 reserved queries, not
independent relevance labels. Only successful references from the 22
development loans are admitted, excluding each query and its provisional
source component. Original/full/compact pools contain 68/82/70 chunks.
Ten distinct scoped indexes per structure are constructed once; query
results are never cached. Structure order rotates and arm order alternates
over three repetitions with a fixed Python hash seed.

\begin{table}[t]
\centering\small
\begin{tabular}{lrrr}
\toprule
Pool & Fresh (s) & Prepared (s) & Reduction \\
\midrule
...
\bottomrule
\end{tabular}
\caption{First-pass retrieval over all 1,508 queries. Prepared time includes
all cold scoped-index builds. Each row compares an identical reference pool;
it does not assess relevance or model inference.}
\label{tab:prepared-retrieval}
\end{table}

All 13,572 original/prepared pairs preserve exact returned rows and repeat
stability, with zero execution errors, exclusion violations or fallback
calls. Independent saved-record checks rehash 2,000 source/text files and
recompute latency aggregates. Original-pool p50/p95 fall from
23.151/26.516 to 10.570/12.112 ms. Cold totals are 159/175/158 ms.
Table~\ref{tab:prepared-retrieval} reports retrieval-only timing on one shared
Windows desktop: PDF compilation overlaps 14.24 seconds of the timed phase.
All observations are retained; no isolated-host or end-to-end claim is made.
Cross-structure source context changes in 1,468/1,508 original--full and
1,221/1,508 original--compact queries. Thus exact prepared-serving agreement
does not justify swapping reference pools. The prepared component is opt-in;
the frozen main runner remains unchanged. There are no additional model calls.

\section{Preserved Historical Material: Not Evidence}
\label{sec:legacy}
The original figures and tables remain as an \emph{author-reconciliation
archive}, not empirical support. All E2--E5 values and plotted coordinates
are unchanged. Their source, model-run, relevance and annotation traces
are not established on the fixed 3,000-source cohort.
...
They are excluded from the main argument and require reconciliation
before submission; preservation does not validate these claims.
\begin{figure*}[t]
\centering
