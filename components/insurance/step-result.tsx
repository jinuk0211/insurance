...
import { PilotResearchPanel } from "@/components/insurance/pilot-research-panel"
...
const TABS: readonly TabDefinition[] = [
  ...
  { id: "terms", label: "약관·위험", shortLabel: "약관", description: "원문 근거와 추출 결과 검토", icon: ShieldCheck },
]
...
function ContractCard({ contract, selectable, selected, onToggle }: {
  ...
  const progress = contract.paidCount !== null && contract.totalPaymentCount
    ...
    : null
  return (
    <article className={`overflow-hidden rounded-[22px] border bg-[#ffffff] shadow-[0_14px_36px_rgba(41,37,31,0.055)] transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[0_18px_42px_rgba(41,37,31,0.09)] ${selected ? "border-[#4f46e5] ring-4 ring-[#4f46e5]/5" : "border-black/8 hover:border-black/20"}`}>
      <div className="flex flex-wrap items-center gap-3 p-4 sm:flex-nowrap sm:gap-4">
        ...
        <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-neutral-950 text-xs font-semibold text-white shadow-[0_8px_20px_rgba(23,33,31,0.18)]" aria-hidden="true">
        ...
        <div className="w-full min-w-[150px] border-t border-black/10 pt-3 text-left sm:w-auto sm:border-l sm:border-t-0 sm:pl-4 sm:pt-0 sm:text-right">
          <p className="text-base font-semibold tabular-nums text-neutral-950">{formatWon(contract.premium)}</p>
...
function OverviewPanel({ model, onNavigate, demoMode }: { model: InsuranceDashboardModel; onNavigate: (tab: DashboardTab) => void; demoMode: boolean }) {
  const metrics = [
    { label: "표시 보험료 합계", value: model.premiumKnownCount ? formatWon(model.totalPremium) : "미표시", note: `정상 계약 ${model.activeCount}건 기준`, icon: BarChart3 },
    { label: "전체 보험 계약", value: `${model.contracts.length}건`, note: `보험사 ${model.insurerCount}곳에서 조회`, icon: FileText },
    { label: "정상 유지 계약", value: `${model.activeCount}건`, note: `실효·해지 ${model.inactiveCount}건`, icon: ShieldCheck },
    { label: "확인이 필요한 항목", value: `${model.dataQuality.unresolvedCount}개`, note: "누락 정보와 약관 근거 검토", icon: FileSearch },
  ]
  return <div className="space-y-7">
    <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><h2 className="text-2xl font-semibold tracking-tight text-slate-950">보험 현황을 한눈에 확인하세요.</h2><p className="mt-2 text-sm text-slate-500">{demoMode ? "샘플 고객의 계약과 확인할 정보를 정리했습니다." : "조회된 계약과 보장 정보를 바탕으로 상담을 준비하세요."}</p></div><span className="inline-flex items-center gap-1.5 self-start rounded-full border border-slate-200 bg-white px-3 py-1.5 text-[11px] text-slate-500"><span className="h-1.5 w-1.5 rounded-full bg-indigo-400" />{demoMode ? "데모 워크스페이스" : "조회 결과"}</span></div>
    <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">{metrics.map(({label,value,note,icon:Icon}) => <section key={label} className="result-surface p-4 sm:p-5"><div className="flex items-center justify-between gap-2"><p className="text-[11px] font-medium text-slate-500 sm:text-xs">{label}</p><Icon className="h-4 w-4 text-slate-400" /></div><p className="mt-4 text-2xl font-semibold tabular-nums tracking-tight text-slate-900 xl:text-[28px]">{value}</p><p className="mt-2 text-[11px] text-slate-400">{note}</p></section>)}</div>
    <div className="grid gap-5 xl:grid-cols-[1.25fr_1fr]">
      <section className="result-surface p-6"><div className="flex items-center justify-between"><h3 className="text-base font-semibold text-slate-900">보장 항목 살펴보기</h3><button onClick={() => onNavigate("diagnosis")} className="inline-flex min-h-9 items-center gap-1 text-xs font-medium text-indigo-600">상세 보기 <ChevronRight className="h-3.5 w-3.5" /></button></div><p className="mt-1 text-xs text-slate-400">계약명과 수집 담보에서 찾은 예비 신호</p><div className="mt-6 flex flex-wrap gap-2">{model.categories.map(category => <button key={category.id} onClick={() => onNavigate("diagnosis")} className={`inline-flex min-h-10 items-center gap-2 rounded-lg border px-3 text-xs ${category.relatedCount ? "border-indigo-100 bg-indigo-50/60 text-indigo-700" : "border-slate-200 bg-slate-50 text-slate-400"}`}>{category.label}<span className="tabular-nums opacity-60">{category.relatedCount}</span></button>)}</div><p className="mt-5 text-[11px] leading-5 text-slate-400">관련 계약이 있어도 실제 가입금액과 지급 조건은 증권·약관에서 확인해야 합니다.</p></section>
      <section className="result-surface p-6"><div className="flex items-center gap-2"><ClipboardCheck className="h-4 w-4 text-indigo-500" /><h3 className="text-base font-semibold text-slate-900">상담 전 체크리스트</h3></div><div className="mt-5 divide-y divide-slate-100">{[{label:"담보 가입금액 보완",value:`${model.dataQuality.coverageAmountMissingCount}개 미수집`,tab:"quality"},{label:"수집 정보와 분류 검토",value:`${model.dataQuality.unresolvedCount}개 확인 필요`,tab:"quality"},{label:"약관 근거와 분석 결과",value:"추출 초안 검토",tab:"terms"}].map(item => <button key={item.label} onClick={() => onNavigate(item.tab as DashboardTab)} className="flex min-h-14 w-full items-center justify-between gap-3 text-left text-xs"><span className="text-slate-600">{item.label}</span><span className="flex items-center gap-2 text-slate-400">{item.value}<ChevronRight className="h-3.5 w-3.5" /></span></button>)}</div></section>
    </div>
    <section className="flex flex-col justify-between gap-5 rounded-2xl border border-indigo-100 bg-indigo-50/60 p-6 sm:flex-row sm:items-center"><div><p className="text-[11px] font-medium text-indigo-500">POLICY RESEARCH</p><h3 className="mt-2 text-lg font-semibold tracking-tight text-indigo-950">추출된 조건, 원문과 함께 확인하세요.</h3><p className="mt-2 text-xs leading-6 text-indigo-900/60">공식 약관 자료실과 Luna 실험 결과에서 지급 조건·면책·감액의 근거를 살펴보세요.</p></div><button onClick={() => onNavigate("terms")} className="inline-flex min-h-11 shrink-0 items-center justify-center gap-2 rounded-xl bg-indigo-600 px-5 text-xs font-semibold text-white shadow-sm transition-colors hover:bg-indigo-700">약관 분석 열기<ChevronRight className="h-4 w-4" /></button></section>
    <section><div className="mb-4 flex items-center justify-between"><div><h3 className="text-base font-semibold text-slate-900">정상 계약</h3><p className="mt-1 text-xs text-slate-400">조회된 계약 중 3건을 먼저 보여드립니다.</p></div><button onClick={() => onNavigate("contracts")} className="inline-flex min-h-10 items-center gap-1 text-xs font-medium text-slate-500">전체 계약<ChevronRight className="h-4 w-4" /></button></div><div className="space-y-3">{model.activeContracts.slice(0,3).map(contract => <ContractCard key={contract.id} contract={contract} />)}{!model.activeContracts.length && <div className="result-surface p-8 text-center text-sm text-slate-500">정상 상태로 확인된 계약이 없습니다.</div>}</div></section>
  </div>
}
  ...
  return (
    <div className="space-y-6">
      <section className="result-surface p-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
          <label className="relative min-w-0 flex-1">
            ...
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="보험사 또는 상품명 검색" className="min-h-11 w-full rounded-xl border border-black/15 bg-white pl-10 pr-3 text-sm outline-none focus:border-[#4f46e5]" />
          </label>
          ...
          <button onClick={onOpenAdditional} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-[#4f46e5] px-4 text-xs font-bold text-[#4338ca] transition-colors hover:bg-[#4f46e5] hover:text-white"><FilePlus2 className="h-4 w-4" aria-hidden="true" />추가 계약 보기</button>
        </div>
      ...
      <div className="space-y-3">
        ...
        {shown.length === 0 && <div className="rounded-[24px] border border-dashed border-black/20 bg-[#ffffff] p-12 text-center"><CircleHelp className="mx-auto h-8 w-8 text-neutral-500" /><p className="mt-3 text-sm font-bold">조건에 맞는 계약이 없습니다.</p><p className="mt-1 text-xs text-neutral-500">검색어나 상태 필터를 바꿔보세요.</p></div>}
      </div>
    </div>
  )
}
  ...
  return (
    <article className={`min-h-[142px] rounded-[20px] border p-4 shadow-[0_10px_26px_rgba(41,37,31,0.045)] ${isRelated ? "border-blue-200 bg-blue-50/80" : "border-neutral-200 bg-neutral-50"}`}>
      <div className="flex items-start justify-between gap-3">
        <div><h3 className="font-semibold text-neutral-950">{category.label}</h3><p className={`mt-1 text-sm font-bold ${isRelated ? "text-blue-700" : "text-neutral-600"}`}>{isRelated ? `관련 계약 ${category.relatedCount}건` : "계약명에서 미확인"}</p></div>
      ...
      <div className="mt-2 flex flex-wrap items-center gap-2">
        {category.knownAmountCount > 0 && <span className="inline-flex rounded-full bg-white px-2.5 py-1 text-[10px] font-semibold text-blue-900 ring-1 ring-blue-200">확인 금액 {formatWon(category.knownAmount)}</span>}
...
function DiagnosisPanel({ model, onOpenTargets }: { model: InsuranceDashboardModel; onOpenTargets: () => void }) {
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><PreliminaryNotice /><button onClick={onOpenTargets} className="inline-flex min-h-11 shrink-0 items-center justify-center gap-2 rounded-xl bg-neutral-950 px-4 text-xs font-bold text-white transition-colors hover:bg-[#4f46e5]"><Settings2 className="h-4 w-4" aria-hidden="true" />검토 기준 설정</button></div>
      {model.groups.map((group) => (
        <section key={group.id} className="result-open-section" aria-labelledby={`group-${group.id}`}>
          <div className="mb-4 flex items-end justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-neutral-400">Coverage group</p><h2 id={`group-${group.id}`} className="mt-1 text-lg font-semibold text-neutral-950">{group.label}</h2></div><span className="text-xs font-bold text-neutral-500">관련 계약 {group.relatedCount}건</span></div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        <section className="result-surface p-5" aria-labelledby="radar-title">
          <h2 id="radar-title" className="text-lg font-semibold">보장군별 확인 금액</h2><p className="mt-1 text-xs text-neutral-500">가입금액이 수집된 담보만 합산하며 미수집 금액은 0원으로 포함하지 않습니다.</p>
          <div className="mt-4 h-[260px] sm:h-[300px]" role="img" aria-label="보장군별 확인 가입금액 레이더 차트"><ResponsiveContainer width="100%" height="100%"><RadarChart data={radarData}><PolarGrid stroke="#d5d0c6" /><PolarAngleAxis dataKey="group" tick={{ fontSize: 11, fill: "#525252" }} /><Radar dataKey="amount" name="확인 금액(만원)" stroke="#4f46e5" fill="#4f46e5" fillOpacity={0.2} strokeWidth={2} /><Tooltip formatter={(value) => `${Number(value).toLocaleString("ko-KR")}만원`} /></RadarChart></ResponsiveContainer></div>
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="result-open-section">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#4338ca]">Consulting worksheet</p><h2 className="mt-1 text-xl font-semibold">비교할 계약을 선택하세요</h2><p className="mt-1 text-xs text-neutral-500">선택 내용은 현재 화면의 비교 워크시트에만 반영됩니다.</p></div><div className="flex flex-wrap gap-2"><button onClick={onOpenAdditional} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-[#4f46e5] bg-white px-4 text-xs font-bold text-[#4338ca] transition-colors hover:bg-[#4f46e5] hover:text-white"><Plus className="h-4 w-4" />기존 계약 선택</button><button onClick={onOpenProposal} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-neutral-950 px-4 text-xs font-bold text-white transition-colors hover:bg-[#4f46e5]"><FilePlus2 className="h-4 w-4" />신규 설계 추가</button></div></div>
...
function PolicyFindingCard({ finding, model }: { finding: InsurancePolicyFinding; model: InsuranceDashboardModel }) {
  ...
  return (
    <article className="result-surface overflow-hidden">
      <div className="flex flex-col gap-3 border-b border-black/10 p-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <span className="bg-neutral-950 px-2 py-1 text-[9px] font-semibold text-white">{companyName}</span>
            <span className="text-[10px] font-semibold text-neutral-500">{contractName}</span>
          </div>
          <h3 className="mt-3 text-lg font-semibold text-neutral-950">{finding.coverage}</h3>
        </div>
        <div className="shrink-0 rounded-[16px] border border-blue-200 bg-blue-50 px-3 py-2 text-right">
          ...
          <p className="mt-0.5 text-lg font-semibold tabular-nums text-blue-950">
            {finding.matchConfidence === null ? "미제공" : `${finding.matchConfidence}%`}
          </p>
        </div>
      </div>
      <dl className="grid sm:grid-cols-2">
        <div className="border-b border-black/10 p-4 sm:border-r"><dt className="text-[10px] font-bold text-[#4338ca]">보험금 지급 조건</dt><dd className="mt-2 text-sm font-semibold leading-6 text-neutral-900">{findingValue(finding.paymentTrigger)}</dd></div>
      ...
      <div className="flex flex-col gap-2 border-t border-black/10 bg-[#f1f5f9] px-4 py-3 text-[10px] sm:flex-row sm:items-center sm:justify-between">
        ...
        <span className={`shrink-0 px-2 py-1 font-semibold ${finding.sourcePage === null ? "bg-neutral-200 text-neutral-600" : "bg-[#4f46e5] text-white"}`}>
  ...
  return (
    <article className={`rounded-[20px] p-4 shadow-[0_12px_30px_rgba(41,37,31,0.05)] ${riskTone(risk)}`}>
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-[9px] font-semibold uppercase tracking-[0.16em]">{risk.severity === "high" ? "중요 경고" : risk.severity === "low" ? "확인 사항" : risk.severity === "medium" ? "주의 사항" : "위험 수준 미제공"}</span>
            {relatedContracts.length > 0 && <span className="truncate text-[9px] opacity-65">{relatedContracts.join(" · ")}</span>}
          </div>
          <h3 className="mt-2 text-base font-semibold leading-6">{risk.title}</h3>
...
function TermsWorkspace({ model, demoMode }: { model: InsuranceDashboardModel; demoMode: boolean }) {
  const [view, setView] = useState<"research" | "customer">(demoMode ? "research" : "customer")
  return <div className="space-y-6">
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div className="inline-flex rounded-xl border border-slate-200 bg-white p-1">
        {([['research', '분석 실험 결과'], ['customer', '고객 계약 근거']] as const).map(([value, label]) => <button key={value} onClick={() => setView(value)} aria-pressed={view === value} className={`min-h-10 rounded-lg px-4 text-xs font-medium ${view === value ? 'bg-indigo-50 text-indigo-700' : 'text-slate-500 hover:bg-slate-50'}`}>{label}</button>)}
      </div>
      <Link href="/insurance/terms" className="inline-flex items-center gap-1 text-xs font-medium text-indigo-600">전체 약관 자료실 <ChevronRight className="h-4 w-4" /></Link>
    </div>
    {view === 'research' ? <PilotResearchPanel /> : <TermsRiskPanel model={model} demoMode={demoMode} />}
  </div>
}
    ...
    return (
      <section className="result-surface p-6 sm:p-10" aria-labelledby="terms-empty-title">
        <div className="mx-auto max-w-xl text-center">
          <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full border border-black/10 bg-[#f1f5f9]"><FileSearch className="h-6 w-6 text-neutral-500" aria-hidden="true" /></div>
          <p className="mt-5 text-[10px] font-semibold uppercase tracking-[0.18em] text-[#4338ca]">No policy evidence attached</p>
          <h2 id="terms-empty-title" className="mt-2 text-xl font-semibold text-neutral-950">연결된 약관 근거가 없습니다</h2>
    ...
    <div className="space-y-6">
      <section className={`rounded-r-[20px] border-l-4 p-4 ${demoMode ? "border-amber-400 bg-amber-50 text-amber-950" : "border-blue-400 bg-blue-50 text-blue-950"}`}>
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="text-sm font-semibold">{demoMode ? "시연용으로 구성한 가상 약관 분석입니다" : "연결된 구조화 약관 분석 결과입니다"}</h2>
      ...
      <section aria-labelledby="policy-findings-title">
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div><p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[#4f46e5]">Policy evidence</p><h2 id="policy-findings-title" className="mt-1 text-xl font-semibold text-neutral-950">특약별 지급조건과 원문 근거</h2></div>
        ...
        <div className="grid gap-4 xl:grid-cols-2">
          ...
          {policyFindings.length === 0 && <div className="col-span-full border border-dashed border-black/20 bg-[#ffffff] p-8 text-center text-sm text-neutral-500">구조화된 지급조건은 제공되지 않았습니다.</div>}
        </div>
      </section>

      <section className="result-open-section" aria-labelledby="change-risk-title">
        <div className="mb-4"><p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[#4338ca]">Replacement risk</p><h2 id="change-risk-title" className="mt-1 text-xl font-semibold text-neutral-950">해지·승환·계약 변경 전 경고</h2><p className="mt-1 text-xs text-neutral-500">신규 가입 권유가 아니라 기존 권리와 보장 공백을 점검하기 위한 검토 항목입니다.</p></div>
        <div className="grid gap-3 lg:grid-cols-2 xl:grid-cols-3">
...
function TargetSettingsDialog({ open, onOpenChange, model, values, onChange, onReset }: { open: boolean; onOpenChange: (open: boolean) => void; model: InsuranceDashboardModel; values: Record<string, string>; onChange: (id: string, value: string) => void; onReset: () => void }) {
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[90dvh] overflow-hidden rounded-[22px] p-0 sm:max-w-3xl"><DialogHeader className="bg-neutral-950 p-5 pr-12 text-white"><DialogTitle>검토 기준 설정</DialogTitle><DialogDescription className="text-neutral-300">자동 산정값이 아닙니다. 상담자가 내부 비교를 위해 입력하는 임시 기준이며 저장되지 않습니다.</DialogDescription></DialogHeader><div className="max-h-[62dvh] overflow-y-auto p-5"><div className="grid gap-3 md:grid-cols-2">{model.categories.map((category) => <label key={category.id} className="rounded-[18px] border border-black/10 bg-[#ffffff] p-3"><span className="flex items-center justify-between text-xs font-bold"><span>{category.label}</span><span className="text-[10px] text-amber-700">상세 확인 필요</span></span><span className="mt-2 flex items-center rounded-xl border border-black/15 bg-white"><input inputMode="numeric" value={values[category.id] ?? ""} onChange={(event) => onChange(category.id, event.target.value.replace(/\D/g, ""))} placeholder="비교 기준 금액" className="min-h-10 min-w-0 flex-1 rounded-xl px-3 text-right text-sm outline-none" aria-label={`${category.label} 내부 비교 기준 금액`} /><span className="px-3 text-xs text-neutral-500">만원</span></span></label>)}</div></div><DialogFooter className="border-t border-black/10 p-4"><button onClick={onReset} className="min-h-10 rounded-xl border border-black/15 px-4 text-xs font-bold">설정 초기화</button><button onClick={() => onOpenChange(false)} className="min-h-10 rounded-xl bg-[#4f46e5] px-6 text-xs font-bold text-white">적용</button></DialogFooter></DialogContent></Dialog>
}
...
export function StepResult({ data, onReset, onLogout, userName, demoMode = false, onConnect, connectionProfile }: Props) {
  ...
  return (
    <div className="insurance-dashboard min-h-screen p-0 lg:p-5 print:p-0">
      <div className="dashboard-frame mx-auto w-full max-w-[1560px] overflow-clip bg-white shadow-[0_8px_40px_rgba(15,23,42,0.04)] ring-1 ring-slate-200/80 print:max-w-none print:rounded-none print:shadow-none print:ring-0">
        <div className="dashboard-layout flex min-h-[calc(100dvh-1.5rem)]">
          <aside className="dashboard-rail result-sidebar hidden w-[224px] shrink-0 flex-col border-r border-slate-200/80 bg-white text-neutral-950 lg:flex print:hidden">
            <Link href="/" className="dashboard-brand flex min-h-[78px] items-center gap-3 border-b border-black/10 px-3 font-semibold">
              <span className="flex h-8 w-8 items-center justify-center rounded-xl bg-indigo-600 text-white">
                <Home className="h-4 w-4" aria-hidden="true" />
              </span>
              <span>
                <span className="block text-base tracking-[-0.03em]">KFin</span>
            ...
            <div className="mx-3 mt-3">
              <div className="dashboard-rail-card rounded-2xl border border-black/10 bg-white p-3">
                <div className="flex items-center gap-3">
                  <div className="flex h-10 w-10 items-center justify-center rounded-full bg-neutral-100 text-sm font-semibold text-neutral-700">{initial}</div>
                  <div className="min-w-0"><p className="truncate text-sm font-semibold">{displayName} 고객</p><p className="text-[10px] text-neutral-500">{demoMode ? "합성 샘플 고객" : "CODEF 조회 고객"}</p></div>
                </div>
            ...
            <nav className="mt-5 px-3" aria-label="보험 대시보드 섹션">
              {TABS.map((item) => {
                const Icon = item.icon
                const selected = tab === item.id
                return (
                  <button key={item.id} aria-pressed={selected} onClick={() => setTab(item.id)} className={`mb-1 flex min-h-11 w-full items-center gap-3 rounded-xl px-3 text-left text-xs font-bold transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 ${selected ? "bg-indigo-50 text-indigo-700" : "text-neutral-600 hover:bg-neutral-100 hover:text-neutral-950"}`}>
            ...
            <div className="mx-3 mt-2">
              <Link href="/insurance/terms" className="mb-2 flex min-h-11 w-full items-center justify-between rounded-xl border border-black/10 bg-white px-3 text-xs font-semibold text-neutral-800 transition-colors hover:border-black/25">
                <span className="flex items-center gap-3"><FileSearch className="h-4 w-4" />약관 자료실</span><ChevronRight className="h-4 w-4" />
              </Link>
              <Link href="/pension" className="flex min-h-11 w-full items-center justify-between rounded-xl border border-black/10 bg-[#17243b] px-3 text-xs font-semibold text-white transition-colors hover:bg-[#4338ca]">
            ...
            <div className="mt-3 border-t border-black/10 px-3 pt-3">
              <div className={`rounded-2xl border p-3 ${demoMode ? "border-slate-200 bg-slate-50" : "border-emerald-200 bg-emerald-50"}`}>
                <div className={`flex items-center gap-2 text-xs font-bold ${demoMode ? "text-slate-600" : "text-emerald-800"}`}><ShieldCheck className="h-4 w-4" aria-hidden="true" />{demoMode ? "시연용 합성 데이터" : "암호화 조회 이력"}</div>
                <p className="mt-1 text-[10px] leading-4 text-neutral-600">{demoMode ? "고객 계약은 시연용입니다. 약관 자료실은 공개 문서를 제공합니다." : "현재 결과는 CODEF 응답 기반입니다."}</p>
              </div>
              {demoMode && onConnect && (
                <button onClick={onConnect} className="mt-2 flex min-h-11 w-full items-center justify-between rounded-2xl bg-neutral-950 px-3 text-left text-[11px] font-semibold text-white transition-colors hover:bg-neutral-800 focus-visible:outline-2 focus-visible:outline-offset-2">
          ...
          <section className="dashboard-content min-w-0 flex-1 bg-slate-50/70 print:overflow-visible">
            <header className="dashboard-header sticky top-0 z-20 border-b border-black/10 bg-white/95 backdrop-blur-xl print:static">
              {demoMode && (
                <div className="demo-banner mx-3 mt-3 flex flex-col gap-2 rounded-[18px] border border-indigo-100 bg-indigo-50/60 px-4 py-2.5 text-[11px] text-indigo-900 sm:flex-row sm:items-center sm:justify-between lg:mx-5">
                  <p><strong className="font-semibold">샘플 고객</strong> · 계약과 금액은 시연용입니다. 약관 자료실에서는 실제 공개 문서를 확인할 수 있습니다.</p>
                  {onConnect && <button onClick={onConnect} className="inline-flex min-h-9 shrink-0 items-center justify-center gap-1 rounded-xl bg-[#4f46e5] px-4 font-semibold text-white transition-colors hover:bg-[#4338ca]">실데이터 CODEF 연결 <ChevronRight className="h-3 w-3" aria-hidden="true" /></button>}
                </div>
              )}
              <div className="dashboard-header-main flex flex-wrap items-center justify-between gap-4 px-5 py-5 lg:px-8">
                <div className="min-w-0">
                  <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#4338ca]">{activeTab.description}</p>
                  <h1 className="mt-1 font-sans text-2xl font-semibold tracking-[-0.035em] text-neutral-950 sm:text-[28px]">{activeTab.label}<span className="ml-3 text-xs font-normal tracking-normal text-slate-400">{displayName} 고객</span></h1>
                </div>
                <div className="flex gap-2 print:hidden"><Link href="/insurance/terms" title="약관 자료실" aria-label="약관 자료실" className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-xl border border-black/15 bg-white/70 px-3 text-neutral-700 transition-colors hover:bg-neutral-950 hover:text-white"><FileSearch className="h-4 w-4" /></Link><ActionButton onClick={() => window.print()} title="현재 대시보드 인쇄"><Printer className="h-4 w-4" /></ActionButton><ActionButton onClick={onReset} title={resetLabel}><RefreshCw className="h-4 w-4" /></ActionButton>{onLogout && <ActionButton onClick={onLogout} title="로그아웃"><LogOut className="h-4 w-4" /></ActionButton>}</div>
              </div>
              <nav className="dashboard-mobile-nav mx-3 flex snap-x gap-2 overflow-x-auto pb-3 lg:hidden print:hidden" aria-label="보험 대시보드 섹션">
                {TABS.map((item) => <button key={item.id} aria-pressed={tab === item.id} onClick={() => setTab(item.id)} className={`min-h-10 shrink-0 snap-start rounded-full px-4 text-xs font-bold transition-colors ${tab === item.id ? "bg-[#17243b] text-white" : "bg-white text-neutral-500 ring-1 ring-black/[0.06]"}`}>{item.shortLabel}</button>)}
              </nav>
            </header>
          ...
          <main id={`panel-${tab}`} aria-label={activeTab.label} className="dashboard-panel p-4 sm:p-6 lg:p-8">
            ...
            {tab === "terms" && <TermsWorkspace model={model} demoMode={demoMode} />}
          </main>
          </section>
        </div>
