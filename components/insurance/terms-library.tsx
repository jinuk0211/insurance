...
type View = "analysis" | "compare" | "files" | "research"
        ...
        <div className="mt-3 space-y-3">
          {section.evidence.map((evidence, index) => (
            <div key={`${evidence.page}-${index}`} className="border-t border-black/10 pt-3 first:border-0 first:pt-0">
              <span className="inline-flex rounded-md bg-[#f1f5f9] px-2 py-1 text-[10px] font-semibold tabular-nums">PDF {evidence.page}쪽</span>
  ...
  return (
    <div className={`rounded-xl border p-3 sm:p-4 ${toneClass}`}>
      <div className="flex flex-wrap items-center justify-between gap-2">
        <strong className="rounded-full bg-white px-2.5 py-1 text-[10px] text-[#17243b] shadow-sm sm:text-[11px]">PDF {evidence.page}쪽</strong>
        <span className="text-[10px] font-semibold text-neutral-500">근거 {section.evidence.length}건 중 대표 문구</span>
      </div>
...
export function TermsLibrary({ initialView = "analysis" }: { initialView?: View }) {
  ...
  return (
    <main className="terms-library min-h-screen bg-slate-50 text-slate-900">
      ...
      {view === "research" && <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-10"><PilotResearchPanel /></div>}
      ...
      {view === "analysis" && (
        <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6" aria-labelledby="analysis-title">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            ...
            <a href={OFFICIAL_POLICY_SOURCE.url} target="_blank" rel="noreferrer" className="inline-flex min-h-10 items-center gap-2 self-start rounded-xl border border-black/10 bg-white px-4 text-xs font-semibold hover:border-black/25">공식 공시 원문 <ArrowUpRight className="h-4 w-4" /></a>
          </div>

          <div className="mt-6 grid gap-3 rounded-2xl border border-black/10 bg-white p-3 lg:grid-cols-[1fr_160px_150px_150px]">
            <label className="relative"><span className="sr-only">보험사, 상품명, 보장 주제 또는 특약 검색</span><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-neutral-400" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="상품명 · 보장 주제 · 특약 검색" className="min-h-11 w-full rounded-xl border border-black/10 bg-[#f8fafc] pl-10 pr-3 text-sm outline-none focus:border-[#4338ca]" /></label>
            ...
            <select value={saleFilter} onChange={(event) => setSaleFilter(event.target.value as SaleFilter)} className="min-h-11 rounded-xl border border-black/10 bg-[#f8fafc] px-3 text-xs font-bold" aria-label="판매 상태 필터"><option value="all">전체 판매 상태</option><option value="on_sale">현재 판매</option><option value="off_sale">과거 판매</option></select>
          </div>
          ...
          <div className="mt-3 grid items-start gap-4 xl:grid-cols-2">
            {filteredRecords.map(({ document, analysis }) => {
              ...
              return (
                <article key={document.id} className="overflow-hidden rounded-3xl border border-black/10 bg-[#ffffff] shadow-[0_12px_35px_rgba(23,33,31,0.06)]">
                  <div className="p-5 sm:p-6">
                    <div className="flex items-start justify-between gap-4">
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2"><span className={`rounded-full px-2.5 py-1 text-[10px] font-semibold ${document.saleStatus === "on_sale" ? "bg-emerald-100 text-emerald-800" : "bg-neutral-100 text-neutral-600"}`}>{document.saleStatus === "on_sale" ? "현재 판매" : "과거 판매"}</span><span className="text-[10px] font-semibold text-[#4338ca]">{document.insurer}</span><span className="text-[10px] font-bold text-neutral-500">{formatDate(document.effectiveFrom)}</span></div>
                    ...
                    <div className="mt-4 flex flex-wrap gap-1.5">
                      {analysis.coverage.topics.length ? analysis.coverage.topics.map((topic) => <span key={topic} className="rounded-lg bg-[#17243b] px-2.5 py-1.5 text-[10px] font-bold text-white">{topic}</span>) : <span className="rounded-lg bg-amber-100 px-2.5 py-1.5 text-[10px] font-bold text-amber-900">보장 주제 자동 미탐지</span>}
                    </div>
                    ...
                    {analysis.riders.names.length > 0 && <p className="mt-4 text-[11px] leading-5 text-neutral-600"><strong className="text-[#17243b]">감지 특약</strong> · {analysis.riders.names.slice(0, 5).join(" / ")}{analysis.riders.names.length > 5 ? ` 외 ${analysis.riders.names.length - 5}개` : ""}</p>}
                  ...
                  <details className="group border-t border-black/10 bg-[#f8fafc]">
                    <summary className="flex min-h-14 cursor-pointer list-none items-center justify-between px-5 text-xs font-semibold sm:px-6">페이지별 원문 근거 펼치기 <ChevronDown className="h-4 w-4 transition-transform group-open:rotate-180" /></summary>
      ...
      {view === "compare" && (
        <section className="mx-auto max-w-[1540px] px-4 py-10 sm:px-6" aria-labelledby="compare-title">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div><p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[#4f46e5]">Official 50 documents</p><h2 id="compare-title" className="mt-1 text-2xl font-semibold">선택 상품 한눈에 비교</h2><p className="mt-2 text-[11px] leading-5 text-neutral-500">가로 스크롤 없이 화면 폭에 맞춰 최대 3개 상품을 나란히 보여줍니다.</p></div>
          ...
          <details className="group mt-5 overflow-hidden rounded-2xl border border-black/10 bg-white">
            <summary className="flex min-h-14 cursor-pointer list-none items-center justify-between gap-3 px-5 text-xs font-semibold">비교할 약관 바꾸기 <span className="flex items-center gap-2 text-[#4f46e5]">50개 목록 열기 <ChevronDown className="h-4 w-4 transition-transform group-open:rotate-180" /></span></summary>
            <div className="border-t border-black/10 p-4 sm:p-5">
              <label className="relative block"><span className="sr-only">비교 문서 검색</span><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-neutral-400" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="상품명 검색" className="min-h-11 w-full rounded-xl border border-black/10 bg-[#f8fafc] pl-10 pr-3 text-sm outline-none focus:border-[#4f46e5]" /></label>
            ...
            <div className="mt-5 overflow-hidden rounded-2xl border border-black/10 bg-white shadow-sm">
              <table className="w-full table-fixed border-collapse text-left text-[10px] leading-4 sm:text-[13px] sm:leading-5">
                ...
                <tbody className="divide-y divide-black/10">
                  ...
                  <tr>
                    <th scope="row" className="bg-blue-50 p-2 align-top font-semibold text-blue-950 sm:p-4">감지 보장 범위</th>
                    {selectedRecords.map(({ document, analysis }) => (
                      <td key={document.id} className="min-w-0 border-l border-black/10 p-2 align-top sm:p-4">
      ...
      {view === "files" && (
        <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6" aria-labelledby="files-title">
          <div><p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[#4338ca]">Source archive</p><h2 id="files-title" className="mt-1 text-2xl font-semibold">PDF · TXT 자료실 50건</h2><p className="mt-2 text-xs leading-5 text-neutral-500">TXT는 다운로드 전용이 아니라 브라우저에서 바로 열립니다. 각 페이지는 <code className="rounded bg-white px-1.5 py-0.5">===== PAGE N =====</code>으로 구분했습니다.</p></div>
          <div className="mt-6 overflow-hidden rounded-2xl border border-black/10 bg-white">
            {POLICY_RECORDS.map(({ document, analysis }, index) => (
              <article key={document.id} className={`grid gap-4 p-4 sm:p-5 lg:grid-cols-[90px_minmax(0,1fr)_170px_auto] lg:items-center ${index ? "border-t border-black/10" : ""}`}>
                <div><span className={`inline-flex rounded-full px-2.5 py-1 text-[10px] font-semibold ${document.saleStatus === "on_sale" ? "bg-emerald-100 text-emerald-800" : "bg-neutral-100 text-neutral-600"}`}>{document.saleStatus === "on_sale" ? "현재 판매" : "과거 판매"}</span><span className="mt-2 block text-[10px] font-bold text-neutral-500">{policyCategory(document.productName)}</span></div>
                <div className="min-w-0"><p className="text-[10px] font-semibold text-[#4338ca]">{document.insurer}</p><h3 className="mt-1 text-sm font-semibold leading-6">{document.productName}</h3><p className="mt-1 text-[10px] text-neutral-500">{document.sourceFileName}</p></div>
